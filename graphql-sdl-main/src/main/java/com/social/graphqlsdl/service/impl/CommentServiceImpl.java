...
private final CommentMapper  commentMapper;
    ...
    public List<CommentDto> getFirstFewCommentsByAuthorId(UUID authorId, Integer count) {
        ...
        return allByAuthor_id.stream().map(
                commentMapper::convertCommentToDTO
    ...
    public List<CommentDto> getFirstFewCommentsByPostId(UUID postId, Integer count) {
        List<Comment> allByAuthor_id = commentRepository.findAllByPost_Id(postId, PageRequest.of(0,count) );
        return allByAuthor_id.stream().map(
                commentMapper::convertCommentToDTO
        ).collect(Collectors.toList());
    }
    ...
    public List<CommentDto> getComments(Integer count, Integer offset) {
        Page<Comment> comments = commentRepository.findAll(PageRequest.of(0,count) );
    ...
    public UUID createComment(CommentDto commentDto) {
        ...
        final Optional<Post> postOptional = postRepository.findById(commentDto.getPostId());
       ...
       Author author= authorOptional.orElseThrow(()->new ResourceNotFound("User is not exist!"));
        ...
        Comment createComment=commentRepository.saveAndFlush(comment);
